...
2. `prototest` - a standalone console app (`src/prototest`) that unit-tests the
   wire protocol, content hash, and interpolation buffer. Built with the same
   v100 toolchain as the plugin (that is the point: it locks the packed-struct
   wire contract of the exact compiler the shipped DLL uses).

The shared packet definitions in `src/netproto/` (`Wire.h`, `Discovery.h`,
`ContentHash.h`) are intentionally plain C++03 (no modern STL).
...
## Part B - prototest (wire-contract unit tests)

`src/prototest` needs the same v100 compiler as the plugin (Part A prereqs), but
no game, KenshiLib, or ENet dependencies - just the CRT. Build and run:
...
It asserts the wire protocol / content hash / interpolation-buffer behavior and
prints a pass count; any assertion failure exits non-zero. Run it after any
change to `src/netproto/` or the sync layer. The scenario regression suite
(`scripts/regress.ps1`, see Parts D-E) uses the full two-client harness instead.
...
3. Touch the game **only** through the `coop::` action facade in
   `src/plugin/test/ScenarioSupport.h` (spawn/teleport/move/positions/squad
   size) - never the engine directly, so all mutation stays SEH-guarded on the
   main thread.
