...
// Session player cap (protocol 56): the host + up to two joins. The transport
...
enum PacketType {
    ...
    PKT_CAM_HINT         = 41,// UNRELIABLE camera center hint (protocol 43 symmetric; protocol 56 host-relayed); CamHintPacket
    ...
    PKT_PEER_STATUS      = 49 // RELIABLE peer roster edge (host -> joins, protocol 56); PeerStatusPacket
};
...
struct HelloPacket {
    ...
    u32 ownRank; // squad-tab rank this join claims (protocol 56; default 1).
...
// Peer roster edge (protocol 56, host -> joins, reliable). The host announces
